...
the existing identity path, including stereo obligations and history. Bond
constraints do not bypass stereo assessment: a terminal route with an unresolved
stereo strategy remains a proposal, even after all required bonds are broken.
...
``write_search_record()`` stores these constraints, each node's remaining required
bonds, and each precursor's target identities in schema ``synplan-tree/4``.
``read_search_record()`` restores them for auditing; older schemas 1–3 load with
empty bond constraints. A record represents a finished search and cannot resume
it. Pickled trees are unsupported.
