...
def cgr_display(cgr: CGRContainer) -> str:
    ...
    _saved_aromatic = CGRContainer.__dict__.get("_CGRContainer__render_aromatic_bond")
    _saved_render_bonds = CGRContainer.__dict__.get("_render_bonds")
    _saved_extra = CGRContainer.__dict__.get("_WideBondDepictCGR__render_aromatic_bond")
    ...
    def _render_atoms(self):
        ...
        for n, atom in self._atoms.items():
            ...
            if (
                ...
                svg.append(
                    f'      <g fill="{atom_color}" font-family="{symbols_font_style}">'
                )
...
def depict_custom_reaction(reaction: ReactionContainer):
    ...
    try:
        ...
        for mol in reaction.molecules():
            ...
            atoms, bonds, _, masks, _, _min_x, min_y, max_x, max_y = mol.depict(
                _embedding=True
            )
    ...
    if sings_plus:
        ...
        for x in sings_plus:
            svg.append(
                f'    <line x1="{x + 0.35:.2f}" y1="0" x2="{x + 0.65:.2f}" y2="0"/>'
            )
            svg.append(
                f'    <line x1="{x + 0.5:.2f}" y1="0.15" x2="{x + 0.5:.2f}" y2="-0.15"/>'
            )
        svg.append("  </g>")
