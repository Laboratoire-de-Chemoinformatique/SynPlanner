...
def molecule_substructure_as_query(mol, atoms) -> QueryContainer:
    ...
    if isinstance(mol, MoleculeContainer):
        ...
        for req in _requirements(mol):
            ...
            for n in req.atoms:
                references.update(mol._bonds[n])
            ...
            if req.kind == "tetrahedron":
                q.atom(req.atoms[0]).stereo = mol._translate_tetrahedron_sign(
                    req.atoms[0], tuple(q._bonds[req.atoms[0]])
                )
            elif req.kind == "allene":
                center, left, right = req.atoms
                env = (
                    next(n for n in q._bonds[left] if n != center),
                    next(n for n in q._bonds[right] if n != center),
                )
                q.atom(center).stereo = mol._translate_allene_sign(center, *env)
            else:
                n, m = req.atoms
                env = (
                    next(k for k in q._bonds[n] if k != m),
                    next(k for k in q._bonds[m] if k != n),
                )
    ...
    *,
    ...
    for index, raw_item in tqdm(
        ...
        try:
            ...
            if stereo_file is not None:
                stereo_file.write(
                    json.dumps(_source_stereo_record(reaction, index), sort_keys=True)
                    + "\n"
                )
            product_smi = str(unite_molecules(reaction.products))
            extracted_rules, skipped = extract_rules(config, reaction)
        ...
        if num_cpus <= 1:
            ...
            with graceful_shutdown() as stop:
                for result in process_pool_map_stream(
                    ...
                    # No worker recycling: max_tasks_per_child can hang in CPython
                    # 3.13/3.14; the pool helper cleans up stale workers at run end.
                ):
                    if stop.is_set():
                        break
