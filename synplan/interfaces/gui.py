    ...
    if res is None or tree is None:
            ...
            with st.spinner("Performing clustering..."):
                try:
                    ...
                    if not current_tree:
                        ...
                        g_cgrs_dict = compose_all_route_cgrs(current_tree) # Assuming this needs the tree
                        st.write("Processing RG-CGRs...")
                        reduced_g_cgrs_dict = compose_all_reduced_route_cgrs(g_cgrs_dict) # Assuming this uses the previous output

                        
                        results = cluster_routes(reduced_g_cgrs_dict, use_strat=False)
        ...
        if st.session_state.get('clustering_done', False):
            ...
            if not clusters or not reactions_dict or not tree:
                ...
                # Display first N clusters
                for cluster_num, group_data in first_items:
                    if not group_data:
                        continue
                    st.markdown(f"**Cluster {cluster_num}** (Size: {group_data['group_size']})")
                    node_id = group_data['node_ids'][0]  # Display the first route as example
                    try:
                        num_steps = len(tree.synthesis_route(node_id))
                        route_score = round(tree.route_score(node_id), 3)
                        svg = get_route_svg(tree, node_id)
                        rg_cgr = group_data['rg_cgr']
                        rg_cgr.clean2d()
                ...
                # Expander for remaining clusters
                if remaining_items:
                ...
                cluster_sizes = [cluster['group_size'] for cluster in clusters.values()]
                cluster_stat_col, cluster_download_col = st.columns(2, gap="medium")

                with cluster_stat_col:
                    st.subheader("Cluster Statistics")
                    if cluster_sizes:
                        cluster_df = pd.DataFrame({'Cluster': clusters.keys(), 'Number of Routes': cluster_sizes})
                        cluster_df.index += 1
                ...
                with cluster_download_col:
                    ...
                    if not tree_for_html:
                        ...
                        for i, (cluster_idx, node_ids) in enumerate(clusters_items):
                        ...
                        # If there are more, use an expander for the rest and ZIP
                        if num_clusters_total > MAX_DOWNLOAD_LINKS_DISPLAYED:
                            ...
                            with st.expander(expander_label):
                                # Individual buttons for remaining clusters
                                for group_index, node_ids in remaining_items:
                                    html_content = routes_clustering_report(
                                        tree_for_html,
                                        clusters_for_html,
                                        str(group_index),
                        ...
                        # ZIP download for all clusters
                        buffer = io.BytesIO()
                        with zipfile.ZipFile(buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                            for idx, ids in clusters_items:
                                html_content = routes_clustering_report(
                                    tree_for_html,
                                    clusters_for_html,
                                    str(idx),
                ...
                if st.button("Run Subclustering Analysis", key="submit_subclustering"):
                    ...
                    with st.spinner("Performing subclustering analysis..."):
                        try:
                            ...
                            rg_cgrs_dict = st.session_state.get('rg_cgrs_dict')
                            g_cgrs_dict = st.session_state.get('g_cgrs_dict')
                            if clusters_for_sub:
                                all_subgroups = subcluster_all_clusters(clusters_for_sub, rg_cgrs_dict, g_cgrs_dict)
                ...
                if st.session_state.get('subclustering_done', False):
                    sub = st.session_state.subclusters
                    tree = st.session_state.tree
                    clusters_for_sub = st.session_state.get('clusters')
                        ...
                        with sub_input_col:
                            ...
                            if not available_cluster_nums:
                                ...
                                if user_input_cluster_num_display in sub:
                                    ...
                                    if not allowed_subclusters:
                                        ...
                                        selected_subcluster = st.selectbox(
                                            ...
                                            key='subcluster_index_select'
                                        )
                                        rg_cgrs = st.session_state.get('rg_cgrs_dict')

                                        subcluster = sub[user_input_cluster_num_display][selected_subcluster]
                                        cluster_rg_cgr = subcluster['rg_cgr']
                                        cluster_rg_cgr.clean2d()
                                        st.image(cluster_rg_cgr.depict(), caption=f"RG-CGR of cluster")

                                else:
                                    st.warning(f"Selected cluster {user_input_cluster_num_display} (index {selected_subcluster}) not found in subclustering results.")
                                ...
                                with sub_display_col:
                                    ...
                                    user_input_cluster_num_display in sub and \
                                    'selected_subcluster' in locals() and \
                                    selected_subcluster in sub[user_input_cluster_num_display]:

                                        subcluster = sub[user_input_cluster_num_display][selected_subcluster]
                                        # st.write(f"Displaying subclusters for **Cluster {user_input_cluster_num_display}.{selected_subcluster}**:")
                                        new_subgroup = post_process_subgroup(subcluster)
                                        new_subgroup['group_lgs'] = group_by_identical_values(subcluster['nodes_data'])
                                        subcluster = new_subgroup

                                        if not subcluster:
                                            st.info("No subclusters found for this selection.")
                                        else:
                                            MAX_ROUTES_PER_SUBCLUSTER = 5
                                            remaining_routes_sub = list(subcluster['nodes_data'].keys())[MAX_ROUTES_PER_SUBCLUSTER:]
                                            st.markdown(f"--- \n**Subcluster** (Size: {len(subcluster['nodes_data'])})")
                                            synthon_reaction = subcluster['synthon_reaction']
                                            st.image(depict_custom_reaction(synthon_reaction), caption=f"Markush-like pseudo reaction of subcluster")
                                            routes_shown = 0
                                            for route_id in subcluster['nodes_data']:
                                                ...
                                                try:
                                                    ...
                                                    if svg_sub:
                                                        st.image(svg_sub, caption=f"Route {route_id}; steps; Score: {route_score_sub}")
                                                    else:
                                            ...
                                            if remaining_routes_sub:
                                                with st.expander(f"... and {len(remaining_routes_sub)} more clusters"):
                                            ...
                                            subcluster_html_content = routes_subclustering_report(tree, subcluster, user_input_cluster_num_display,
                                                                                                selected_subcluster, rg_cgrs, if_lg_group=True)
                                            st.download_button(
                                                label=f"Download report for cluster {user_input_cluster_num_display}.{selected_subcluster}",
                                                data=subcluster_html_content,
                                                file_name=f"subcluster_{user_input_cluster_num_display}.{selected_subcluster}_{st.session_state.target_smiles}.html",
                                                mime="text/html"
                                            )
