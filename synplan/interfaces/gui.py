...
def initialize_app():
    ...
    if "target_smiles" not in st.session_state:
        st.session_state.target_smiles = '' # Initial value, might be overwritten by ketcher
...
def setup_sidebar():
    ...
    st.sidebar.markdown("https://github.com/Laboratoire-de-Chemoinformatique/SynPlanner/tree/main/tutorials")

    st.sidebar.title("Paper")
    st.sidebar.markdown("https://chemrxiv.org/engage/chemrxiv/article-details/66add90bc9c6a5c07ae65796")
...
def handle_molecule_input():
    ...
    molecule_text_input = st.text_input("SMILES:", value=st.session_state.ketcher, key="smiles_text_input_key")
    # When text input changes, update ketcher state and vice-versa
    # This is a bit tricky with Streamlit's rerun model.
    # We will primarily rely on st_ketcher to be the source of truth for smile_code
    # And if text input changes, it will become the new value for ketcher.

    smile_code_ketcher = st_ketcher(molecule_text_input, key="ketcher_widget")

    # If smile_code_ketcher is different from molecule_text_input, it means ketcher was updated.
    # If molecule_text_input changes, it becomes the input to ketcher.
    # The smile_code_ketcher will be the one used for processing.
    ...
    if 'target_smiles' in st.session_state and current_smile_code != st.session_state.target_smiles:
        ...
        st.session_state.reactions_dict = None
        st.session_state.subclusters = None
        # Update ketcher session state if needed, though st_ketcher might handle its own internal state
        st.session_state.ketcher = current_smile_code


    return current_smile_code # Return the active SMILES code
...
def setup_planning_options():
    ...
    st.markdown(f"The molecule SMILES is actually: ``{st.session_state.get('ketcher', DEFAULT_MOL)}``")
    ...
    with col_options_1:
        search_strategy_input = st.selectbox(label='Search strategy', options=('Expansion first', 'Evaluation first',), index=0, key="search_strategy_input")
        ucb_type = st.selectbox(label='UCB type', options=('uct', 'puct', 'value'), index=0, key="ucb_type_input") # Fixed label
        c_ucb = st.number_input("C coefficient of UCB", value=0.1, placeholder="Type a number...", key="c_ucb_input")

    with col_options_2:
        max_iterations = st.slider('Total number of MCTS iterations', min_value=50, max_value=1000, value=300, key="max_iterations_slider")
        max_depth = st.slider('Maximal number of reaction steps', min_value=3, max_value=9, value=6, key="max_depth_slider")
        min_mol_size = st.slider('Minimum size of a molecule to be precursor', min_value=0, max_value=7, value=0,
                                  key="min_mol_size_slider", help='Number of non-hydrogen atoms in molecule')
    ...
    if st.button('Start retrosynthetic planning', key="submit_planning_button"):
        ...
        st.session_state.g_cgrs_dict = None
        st.session_state.rg_cgrs_dict = None
        active_smile_code = st.session_state.get('ketcher', DEFAULT_MOL) # Get current SMILES
        st.session_state.target_smiles = active_smile_code # Store the SMILES used for this run
        ...
        try:
            ...
            if target_molecule is None:
                ...
                building_blocks_path, ranking_policy_weights_path, reaction_rules_path = load_planning_resources_cached()
                with st.spinner("Running retrosynthetic planning..."):
                    ...
                    for step, (solved, node_id) in enumerate(tree):
                        progress_value = min(1.0, (step + 1) / planning_params["max_iterations"])
                        mcts_bar.progress(progress_value, text=f"{mcts_progress_text} ({step+1}/{planning_params['max_iterations']})")
    ...
    if st.session_state.get('planning_done', False):
        ...
        if res.get("solved", False): # Use .get for safety
            ...
            with stat_col:
                ...
                try:
                     if 'target_smiles' not in res and 'target_smiles' in st.session_state: res['target_smiles'] = st.session_state.target_smiles
...
def download_planning_results():
    ...
    if st.session_state.get('planning_done', False) and st.session_state.res and st.session_state.res.get("solved", False):
        res = st.session_state.res
        tree = st.session_state.tree
        ...
        try:
            ...
            dl_html = download_button(html_body, f'results_synplanner_{st.session_state.target_smiles}.html', 'Download results (HTML)')
            if dl_html: st.markdown(dl_html, unsafe_allow_html=True)

            try:
                res_df = pd.DataFrame(res, index=[0])
                dl_csv = download_button(res_df, f'stats_synplanner_{st.session_state.target_smiles}.csv', 'Download statistics (CSV)')
        ...
        if st.button('Run Clustering', key='submit_clustering_button'):
            ...
            with st.spinner("Performing clustering..."):
                try:
                    ...
                    results = cluster_routes(reduced_g_cgrs_dict, use_strat=False) # num_clusters was removed from args
                    ...
                    if st.session_state.clusters is not None and st.session_state.reactions_dict is not None: # Check for None explicitly
...
def display_clustering_results():
    ...
    if st.session_state.get('clustering_done', False):
        ...
        if clusters is None or tree is None: # reactions_dict removed as not critical for display part
            st.error("Clustering results (clusters or tree) are missing. Please re-run clustering.")
            st.session_state.clustering_done = False
            return
        ...
        for cluster_num, group_data in first_items:
            if not group_data or 'node_ids' not in group_data or not group_data['node_ids']:
                st.warning(f"Cluster {cluster_num} has no data or node_ids.")
                continue
            st.markdown(f"**Cluster {cluster_num}** (Size: {group_data.get('group_size', 'N/A')})")
            ...
            try:
                ...
                if svg and rg_cgr_svg:
                    ...
                    st.image(svg, caption=f"Route {node_id}; {num_steps} steps; Route score: {route_score}")
                    st.warning(f"RG-CGR could not be displayed for cluster {cluster_num}.")
                else:
                    st.warning(f"Could not generate SVG for route {node_id} or its RG-CGR.")
            except Exception as e:
                st.error(f"Error displaying route {node_id} for cluster {cluster_num}: {e}")

        if remaining_items:
            with st.expander(f"... and {len(remaining_items)} more clusters"):
                for cluster_num, group_data in remaining_items:
                    if not group_data or 'node_ids' not in group_data or not group_data['node_ids']:
                        st.warning(f"Cluster {cluster_num} in expansion has no data or node_ids.")
                        continue
                    st.markdown(f"**Cluster {cluster_num}** (Size: {group_data.get('group_size', 'N/A')})")
                    node_id = group_data['node_ids'][0]
                    try:
...
def download_clustering_results():
    ...
    if st.session_state.get('clustering_done', False):
        ...
        rg_cgrs_for_html = st.session_state.get('rg_cgrs_dict') # This was used instead of reactions_dict in the original for report
        ...
        for i, (cluster_idx, group_data) in enumerate(clusters_items): # group_data might not be needed here if report uses cluster_idx
        ...
        if num_clusters_total > MAX_DOWNLOAD_LINKS_DISPLAYED:
            ...
            with st.expander(expander_label):
                for group_index, _ in remaining_items: # group_data not needed here either
                    try:
                        html_content = routes_clustering_report(
                            tree_for_html,
                            clusters_for_html,
                        ...
                        st.error(f"Error generating report for cluster {group_index} (expanded): {e}")
...
def setup_subclustering():
    ...
    if st.session_state.get('clustering_done', False): # Subclustering depends on clustering being done
        st.divider()
        ...
        if st.button("Run Subclustering Analysis", key="submit_subclustering_button"):
            ...
            with st.spinner("Performing subclustering analysis..."):
                try:
                    ...
                    rg_cgrs_dict_for_sub = st.session_state.get('rg_cgrs_dict') # Corrected name
                    g_cgrs_dict_for_sub = st.session_state.get('g_cgrs_dict')   # Corrected name

                    if clusters_for_sub and rg_cgrs_dict_for_sub and g_cgrs_dict_for_sub: # Ensure all are present
                        all_subgroups = subcluster_all_clusters(clusters_for_sub, rg_cgrs_dict_for_sub, g_cgrs_dict_for_sub)
                        ...
                        missing = []
                        if not clusters_for_sub: missing.append("clusters")
                        if not rg_cgrs_dict_for_sub: missing.append("RG-CGRs dictionary")
                        if not g_cgrs_dict_for_sub: missing.append("G-CGRs dictionary")
                        st.error(f"Cannot run subclustering. Missing data: {', '.join(missing)}. Please ensure clustering ran successfully.")
                        st.session_state.subclustering_done = False
...
def display_subclustering_results():
    ...
    if st.session_state.get('subclustering_done', False):
        ...
        if not sub or not tree:
            st.error("Subclustering results (subclusters or tree) are missing. Please re-run subclustering.")
            st.session_state.subclustering_done = False
            return
        ...
        with sub_input_col:
            ...
            if user_input_cluster_num_display in sub:
                ...
                if not allowed_subclusters_indices:
                    st.warning(f"No reaction steps (subclusters) found for Cluster {user_input_cluster_num_display}.")
                else:
                    ...
                    if selected_subcluster_idx in sub[user_input_cluster_num_display]:
                        current_subcluster_data = sub[user_input_cluster_num_display][selected_subcluster_idx]
                        if 'rg_cgr' in current_subcluster_data:
                            cluster_rg_cgr_display = current_subcluster_data['rg_cgr']
                            cluster_rg_cgr_display.clean2d()
                            st.image(cluster_rg_cgr_display.depict(), caption=f"RG-CGR of parent Cluster {user_input_cluster_num_display}")
                        else:
                            st.warning("RG-CGR for this subcluster not found.")
            else:
                st.warning(f"Selected cluster {user_input_cluster_num_display} not found in subclustering results.")
                return 

        with sub_display_col:
            st.subheader("Subcluster Details")
            if user_input_cluster_num_display in sub and \
               selected_subcluster_idx in sub[user_input_cluster_num_display]:

                subcluster_content = sub[user_input_cluster_num_display][selected_subcluster_idx]

                subcluster_to_display = post_process_subgroup(subcluster_content)

                if not subcluster_to_display or 'nodes_data' not in subcluster_to_display or not subcluster_to_display['nodes_data']:
                    st.info("No routes or data found for this subcluster selection.")
                else:
                    MAX_ROUTES_PER_SUBCLUSTER = 5
                    all_route_ids_in_subcluster = list(subcluster_to_display['nodes_data'].keys())
                    routes_to_display_direct = all_route_ids_in_subcluster[:MAX_ROUTES_PER_SUBCLUSTER]
                    remaining_routes_sub = all_route_ids_in_subcluster[MAX_ROUTES_PER_SUBCLUSTER:]
                    ...
                    if 'synthon_reaction' in subcluster_to_display:
                        ...
                        try:
                            st.image(depict_custom_reaction(synthon_reaction), caption=f"Markush-like pseudo reaction of subcluster") # Assuming depict_custom_reaction
                    ...
                    for route_id in routes_to_display_direct:
                        try:
                            ...
                            if svg_sub:
                                st.image(svg_sub, caption=f"Route {route_id}; Score: {route_score_sub}")
                            else:
                                st.warning(f"Could not generate SVG for route {route_id}.")
                        except Exception as e:
                            st.error(f"Error displaying route {route_id} in subcluster: {e}")

                    if remaining_routes_sub:
                        with st.expander(f"... and {len(remaining_routes_sub)} more routes in this subcluster"):
                            for route_id in remaining_routes_sub:
                                try:
                                    ...
                                    st.error(f"Error displaying route {route_id} in subcluster (expanded): {e}")
            else:
...
def download_subclustering_results():
    ...
    if st.session_state.get('subclustering_done', False) and \
       'subcluster_num_select_key' in st.session_state and \
       'subcluster_index_select_key' in st.session_state:

        sub = st.session_state.get('subclusters')
        tree = st.session_state.get('tree')
        rg_cgrs_for_report = st.session_state.get('rg_cgrs_dict') # Used by routes_subclustering_report
        ...
        if not tree or not sub or not rg_cgrs_for_report:
            st.warning("Missing data for subclustering report generation (tree, subclusters, or RG-CGRs).")
            return

        if user_input_cluster_num_display in sub and \
           selected_subcluster_idx in sub[user_input_cluster_num_display]:

            subcluster_data_for_report = sub[user_input_cluster_num_display][selected_subcluster_idx]
            # Apply the same post-processing as in display
            processed_subcluster_data = post_process_subgroup(subcluster_data_for_report)
            if 'nodes_data' in subcluster_data_for_report and isinstance(subcluster_data_for_report['nodes_data'], dict):
                processed_subcluster_data['group_lgs'] = group_by_identical_values(subcluster_data_for_report['nodes_data'])
            else:
            ...
            try:
                ...
                st.error(f"Error generating download report for subcluster {user_input_cluster_num_display}.{selected_subcluster_idx}: {e}")
        # else:
...
def implement_restart():
    ...
    if st.button("Clear All Results & Restart", key="restart_button"):
        keys_to_clear = [
            ...
            "clustering_done", "clusters", "reactions_dict", "num_clusters_setting",
            "g_cgrs_dict", "rg_cgrs_dict",
            "subclustering_done", "subclusters", # "sub" was renamed
            "clusters_downloaded",
            #Potentially ketcher related keys if they need manual reset beyond new input
            "ketcher_widget", "smiles_text_input_key", # Keys for widgets
            "subcluster_num_select_key", "subcluster_index_select_key"
        ]
...
def main():
    ...
    if st.session_state.get('planning_done', False):
        ...
        if st.session_state.res and st.session_state.res.get("solved", False):
            stat_col, download_col = st.columns(2, gap="medium") # Placeholder for download column
            with stat_col:
    ...
    if st.session_state.get('planning_done', False) and st.session_state.res and st.session_state.res.get("solved", False):
        ...
        if st.session_state.get('clustering_done', False):
            ...
            with cluster_stat_col:
                clusters = st.session_state.clusters
                cluster_sizes = [cluster.get('group_size', 0) for cluster in clusters.values() if cluster] # Safe get
                st.subheader("Cluster Statistics")
                if cluster_sizes:
                    cluster_df = pd.DataFrame({'Cluster': [k for k,v in clusters.items() if v], # Filter out empty clusters
                                            'Number of Routes': [v['group_size'] for v in clusters.values() if v]})
