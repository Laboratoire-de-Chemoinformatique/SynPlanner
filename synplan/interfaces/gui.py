...
TREE_ALGORITHM_OPTIONS = {
    "UCT": "uct",
    "Beam": "beam",
    "BestFirst": "best_first",
    "BreadthFirst": "breadth_first",
    "LazyNestedMonteCarlo": "lazy_nmcs",
    "NestedMonteCarlo": "nmcs",
}
...
def get_base_gui():
    ...
    if _BASE_GUI is None:
        import synplan.interfaces.gui_support as base_gui
...
def initialize_app():
    ...
    if "planning_algorithm" not in st.session_state:
        st.session_state.planning_algorithm = TREE_ALGORITHM_OPTIONS["UCT"]
    if "planning_algorithm_label" not in st.session_state:
        st.session_state.planning_algorithm_label = "UCT"
...
def setup_planning_options():
    ...
    with col_options_1:
        tree_algorithm_input = st.selectbox(
            label="Tree algorithm",
            options=tuple(TREE_ALGORITHM_OPTIONS),
            index=0,
            key="tree_algorithm_input",
        )
    ...
    tree_algorithm = TREE_ALGORITHM_OPTIONS[tree_algorithm_input]

    planning_params = {
        "algorithm": tree_algorithm,
        "algorithm_label": tree_algorithm_input,
    ...
    if st.button("Start retrosynthetic planning", key="submit_planning_button"):
        ...
        st.session_state.planning_algorithm = planning_params["algorithm"]
        st.session_state.planning_algorithm_label = planning_params["algorithm_label"]
            ...
            if target_molecule is None:
                ...
                with st.spinner("Running retrosynthetic planning..."):
                    ...
                    tree_config = base_gui.TreeConfig(
                        algorithm=planning_params["algorithm"],
                    ...
                    search_progress_text = (
                        f"Running {planning_params['algorithm_label']} iterations..."
                    )
                    search_bar = st.progress(0, text=search_progress_text)
                    for step, (_solved, _route_id) in enumerate(tree):
                        ...
                        search_bar.progress(
                            progress_value,
                            text=f"{search_progress_text} ({step+1}/{planning_params['max_iterations']})",
                        )
...
def download_planning_results():
    ...
    if st.session_state.get("expansion_visualization_html"):
        algorithm = st.session_state.get(
            "planning_algorithm",
            TREE_ALGORITHM_OPTIONS["UCT"],
        )
        st.download_button(
            label="Download expansion timeline (HTML)",
            data=st.session_state.expansion_visualization_html,
            file_name=f"{algorithm}_tree_exp_{st.session_state.target_smiles}.html",
            mime="text/html",
...
def _generate_expansion_visualization_in_process(
    ...
    try:
        ...
        from synplan.utils.tree_visualization.visualize_tree_main import (
...
def generate_expansion_visualization_html(
    ...
    with tempfile.TemporaryDirectory(prefix="synplan_expansion_") as temp_dir:
        ...
        command = [
            sys.executable,
            "-m",
            "synplan.utils.tree_visualization.visualize_tree_main",
            "--tree",
...
def process_tree_visualization_section():
    ...
    elif not st.session_state.get("tree_visualization_html"):
        try:
            with st.spinner(
                ...
                st.session_state.tree_visualization_html = (
                    generate_expansion_visualization_html(
                        tree,
                        clusters=clusters,
                        tree_pickle_bytes=st.session_state.get("tree_pickle_bytes"),
                    )
                )
