    ...
    for value in batch:
        try:
            ...
            if isinstance(molecule, MoleculeContainer):
                output.append(str(safe_canonicalization(molecule, clean_stereo=False)))
        except Exception:
            pass
    return output
...
def load_building_block(
    ...
    *,
    ...
    """Load a validated typed ordinary building-block stock.

    This loading facade keeps file I/O at the established ``synplan.utils.loading``
    boundary while delegating stock validation and identity handling to the
    building-block domain implementation.
    """
    from synplan.chem.building_blocks.stock import load_building_block_stock

    return load_building_block_stock(building_blocks_path, config=config)
