...
def _build_payload(
    ...
    *,
    ...
    for node_id in sorted(nodes_depth_tree.keys()):
        ...
        parent_tree_id = int(tree.parents.get(node_id, 0))
        parent_pos = (
        ...
        molecule_keys = [str(molecule) for molecule in molecules]
...
_TREE_PAGE_CSS = """\
      ...
      .edges line {
        stroke: var(--edge);
        stroke-width: __EDGE_STROKE_WIDTH__;
        opacity: 0;
        pointer-events: none;
      }
      ...
      .edges line.visible.winning-active {
        ...
        stroke-width: __WINNING_EDGE_STROKE_WIDTH__;
      ...
      .nodes circle {
        ...
        stroke-width: __NODE_STROKE_WIDTH__;
      ...
      .nodes circle.active {
        ...
        stroke-width: __ACTIVE_NODE_STROKE_WIDTH__;
...
# Keep this as plain JavaScript with marker replacement instead of an f-string.
# That lets the embedded object literals and template strings use normal braces.
_TREE_PAGE_SCRIPT = """\
      const nodes = __NODES_JSON__;
      const edges = __EDGES_JSON__;
      const stats = __STATS_JSON__;
      const winningPaths = __WINNING_PATHS_JSON__;
      const steps = __STEPS_JSON__;
      const viewBox = __VIEW_BOX_JSON__;
      const clusters = __CLUSTERS_JSON__;
      const routeNodes = __ROUTE_NODES_JSON__;
      const targetSmiles = __TARGET_SMILES_JSON__;
...
def _json_for_browser(value: object) -> str:
    return json.dumps(value, ensure_ascii=True)


def _build_browser_payload(payload: dict[str, object]) -> dict[str, str]:
    return {
        "nodes": _json_for_browser(payload["nodes"]),
        "edges": _json_for_browser(payload["edges"]),
        "stats": _json_for_browser(payload["stats"]),
        "winning_paths": _json_for_browser(payload.get("winningPaths", [])),
        "steps": _json_for_browser(payload["steps"]),
        "view_box": _json_for_browser(payload["viewBox"]),
        "clusters": _json_for_browser(payload.get("clusters", [])),
        "route_nodes": _json_for_browser(payload.get("routeNodes", {})),
        "target_smiles": _json_for_browser(payload.get("targetSmiles", "")),
    }


def _build_molecule_svg_records(payload: dict[str, object]) -> str:
    molecule_svgs = payload.get("moleculeSvgs", {}) or {}
    return "\n".join(
...
def _render_styles(node_radius: float) -> str:
    replacements = {
        "__EDGE_STROKE_WIDTH__": f"{min(max(node_radius * 0.5, 0.6), 6.0):.3f}",
        "__WINNING_EDGE_STROKE_WIDTH__": f"{min(max(node_radius * 0.75, 1.4), 10.0):.3f}",
        "__NODE_RADIUS__": f"{node_radius:.6f}",
        "__NODE_STROKE_WIDTH__": f"{min(max(node_radius * 0.35, 0.8), 8.0):.3f}",
        "__ACTIVE_NODE_STROKE_WIDTH__": f"{min(max(node_radius * 0.6, 1.2), 10.0):.3f}",
    }
    styles = _TREE_PAGE_CSS
    for marker, value in replacements.items():
        styles = styles.replace(marker, value)
    return styles


def _render_script(browser_payload: dict[str, str], node_radius: float) -> str:
    replacements = {
        "__NODES_JSON__": browser_payload["nodes"],
        "__EDGES_JSON__": browser_payload["edges"],
        "__STATS_JSON__": browser_payload["stats"],
        "__WINNING_PATHS_JSON__": browser_payload["winning_paths"],
        "__STEPS_JSON__": browser_payload["steps"],
        "__VIEW_BOX_JSON__": browser_payload["view_box"],
        "__CLUSTERS_JSON__": browser_payload["clusters"],
        "__ROUTE_NODES_JSON__": browser_payload["route_nodes"],
        "__TARGET_SMILES_JSON__": browser_payload["target_smiles"],
        "__NODE_RADIUS_WORLD__": f"{node_radius:.12f}",
    }
    script = _TREE_PAGE_SCRIPT
    for marker, value in replacements.items():
...
def _render_html(
    *,
    styles: str,
    script: str,
    molecule_svg_records: str,
    target_svg: str,
) -> str:
    target_markup = (
        target_svg
        if target_svg
        else "<div class='muted'>Target depiction unavailable.</div>"
    )
...
def generate_expansion_html(
    ...
    *,
    ...
    html = _render_html(
        styles=_render_styles(float(payload["nodeRadius"])),
        script=_render_script(
            _build_browser_payload(payload),
            float(payload["nodeRadius"]),
        ),
        molecule_svg_records=_build_molecule_svg_records(payload),
        target_svg=str(payload.get("targetSvg", "")),
    )
