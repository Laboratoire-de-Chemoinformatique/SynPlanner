...
1. **Score for a single-step route that the scanner can process must not
   collapse a route the scanner *cannot* process to the same value.** This
   is a property the scorer must surface: either via an explicit
   "n_processed" counter, a typed return, or a deviation in the score.

2. **Static guard** against the antipattern of dividing by ``len(route)``
   without tracking processed-step count. A clean fix introduces a counter
...
def test_failed_scan_does_not_yield_perfect_score():
    ...
    """
    try:
        from synplan.route_quality.protection.scanner import RouteScanner
    ...
    with patch.object(scanner, "scan_route", return_value=([], 0)):
