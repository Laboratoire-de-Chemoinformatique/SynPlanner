...
def test_frozen_bond_preserves_stereo_proposal_obligations(tmp_path):
    from synplan.mcts.record import read_search_record, write_search_record

    # Lactic acid reduction requires selectivity evidence even if the C-O adjacency is frozen.
    target = smiles("C[C@H](O)C(=O)O")
    rule = CanonicalRetroReactor.from_smarts("[C:1]-[O:2]>>[C:1]=[O:2]")
    tree = _tree(
        target, [rule], {(2, 3): 2}, building_blocks={str(smiles("CC(=O)C(=O)O"))}
    ).run()
    ...
    assert node.stereo_obligations
    assert node.new_precursors[0].target_atom_provenance.pairs
    record = read_search_record(write_search_record(tree, tmp_path / "stereo.json"))
    restored = record.nodes[tree.proposal_nodes[0]]
    assert restored.stereo_obligations.key == node.stereo_obligations.key
    assert (
        restored.new_precursors[0].target_atom_provenance
        == node.new_precursors[0].target_atom_provenance
    )
    assert not restored.is_solved()
