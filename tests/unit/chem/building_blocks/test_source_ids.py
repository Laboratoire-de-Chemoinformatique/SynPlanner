...
def test_material_variants_keep_source_ids_and_do_not_sell_a_racemate_as_an_enantiomer(
    ...
    molecule = smiles("C[C@H](O)C(=O)O")
    key = inchi_key(molecule)
    source = {"vendor": "MC", "id": "MCULE-1234567890", "smiles": str(molecule)}
    ...
    assert payload["rows"][0] == [
        "MC",
        {"text": source["id"], "href": "https://mcule.com/MCULE-1234567890/"},
    ]
...
def test_unpriced_sources_merge_and_compressed_sqlite_opens_without_chemistry(
    ...
    with raw.open("w", newline="") as handle:
        ...
        for source in sources:
            writer.writerow(("C1CCCC1", json.dumps([source])))
...
def test_molport_cxsmiles_source_is_not_truncated_or_promoted(tmp_path):
    ...
    assert record.sources[0]["smiles"] == raw_smiles
