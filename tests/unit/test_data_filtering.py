...
def test_ccsp3_with_break():
    ...
    rxn = smiles("[CH3:1][CH2:2][CH2:3][CH3:4]>>[CH3:1][CH2:2][CH3:3].[CH3:4]")  # Breaking C-C bond in butane
    f = CCsp3BreakingFilter()
    assert f(rxn)
...
def test_dynamic_bonds_too_many():
    ...
    rxn = smiles("[CH2:1]=[CH:2]-[CH:3]=[CH:4]-[CH:5]=[O:6]>>[CH2:1]-[CH2:2]-[CH2:3]-[CH2:4]-[CH2:5]-[O:6]")  # 5 dynamic bonds
    f = DynamicBondsFilter(min_bonds_number=1, max_bonds_number=2)
    assert f(rxn)
...
def test_wrong_ch_breaking():
    ...
    rxn = smiles("[CH3:1][CH2:2][H:3]>>[CH3:1][CH2:2][CH3:4]")  # Breaking C-H and forming C-C incorrectly
    f = WrongCHBreakingFilter()
    assert f(rxn)

def test_cc_ring_breaking():
    # Test a reaction with C-C ring breaking
    rxn = smiles("[CH2:1]1[CH2:2][CH2:3][CH2:4][CH2:5]1>>[CH2:1]-[CH2:2]-[CH2:3]-[CH2:4]-[CH2:5]")  # Breaking cyclopentane ring
    f = CCRingBreakingFilter()
    assert f(rxn)

def test_cc_ring_no_breaking():
    # Test a reaction without C-C ring breaking but with other changes
    rxn = smiles("[CH2:1]1[CH2:2][CH2:3][CH2:4][CH2:5]1.[O:6]>>[CH2:1]1[CH2:2][CH2:3][CH2:4][CH2:5]1.[OH:6]")  # No ring breaking, just adding H to O
    f = CCRingBreakingFilter()
    assert not f(rxn)
